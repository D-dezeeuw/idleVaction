...
// that assert "the beat fires the moment its gate is met" mean "once enough time passes for
// every READY beat" — so drain the queue, advancing the valve window each round. The gate
// LOGIC under test is unchanged; only the pacing layer is stepped past, exactly as real ticks do.
const fireStory = st => {
  for (let i = 0; i < 40; i++) {
    const before = st.story.seen.length;
    st.stats.runSec = (st.stats.runSec || 0) + (C.STORY_VALVE_SEC || 0) + 1;
    E.checkStory(st);
    if (st.story.seen.length === before) break;
  }
};
...
const miniF = n => 1 + C.MILESTONE_MINI.bonus * Math.floor(n / C.MILESTONE_MINI.every);
ok(approx(M.milestoneMult(10), 2 * miniF(10)), 'first milestone doubling at 10 buys (×2, mini-milestone factor on top)');
...
ok(approx(M.unitCost(0, 9), 1778.8181474549995), 'unitCost(0,9) matches the golden snapshot (Phase-C refit: growth[0] 1.7)');
ok(approx(M.unitCost(0, 10), 3023.9908506734996), 'unitCost(0,10) matches the golden snapshot (Phase-C refit)');
...
ok(approx(M.milestoneMult(9), 1 * miniF(9)), 'milestoneMult(9): no doubling yet, one mini-milestone (buy 5) in the factor');
ok(approx(M.milestoneMult(10), 2 * miniF(10)), 'milestoneMult(10): the first ×2 lands exactly at buy 10 (× the mini factor)');
ok(approx(M.milestoneMult(50), 24 * miniF(50)), 'milestoneMult(50): soft-capped tail past the knee (16·2.5=... base 24) × the mini factor');
...
const a = ST.newGame();
a.stats.lifetimeCashThisTree = 1e12;   // a big run
a.stats.runSec = 300;
...
console.log('\n[7b] buyAmenity progression');
{
  ...
  ok(approx(cost0, bugSpray.costBase * C.AMENITY.costScale), 'level-0 cost equals costBase × the refit costScale');
...
{
  // Above-floor Comfort refit (config.COMFORT.floorFrac=1, docs/05 §9.3): comfortMultiplier reads
  // EFFECTIVE Comfort = max(0, Comfort − floorFrac·comfortFloor), where comfortFloor = accScore(tier)
  // ·wAcc. These bare probes carry no accommodation, so comfortFloor defaults to accScore(0)·wAcc = 50
  // — inputs at/below 50 clamp to L=1, and the snapshots below are the post-refit values (MULT 0.47,
  // C0 8). The floorFrac=0 equivalence to the pre-refit formula is pinned in [26b] below.
  const probe = ST.newGame();
  ...
  ok(M.comfortMultiplier({ _comfortCache: C0 }) === 1, 'comfortMultiplier(C0=8) == 1 — below the tier-0 floor (50), effective Comfort clamps to 0');
  ok(approx(M.comfortMultiplier({ _comfortCache: 10 * C0 }), 1.3180459965236873), 'comfortMultiplier(10·C0=80, above the floor) matches the post-refit golden snapshot');
  ok(approx(M.comfortMultiplier({ _comfortCache: 100 * C0 }), 1.9289922327608515), 'comfortMultiplier(100·C0=800) matches the post-refit golden snapshot');
...
// ---------- 26b. Above-floor Comfort refit (docs/05 §9.3): the income multiplier reads
// player-EARNED (above-floor) Comfort while the tier GATE still reads TOTAL Comfort. Pins the
// three load-bearing properties: the floorFrac=0 legacy escape hatch, tier-independent
// above-floor behavior (no check-in penalty), and gate bridgeability. ----------
console.log('\n[26b] above-floor Comfort refit: floorFrac escape hatch, above-floor income, gate reads total');
{
  // (a) floorFrac=0 ESCAPE HATCH: bit-identical to the pre-refit total-Comfort formula, at ANY tier.
  const savedFF = C.COMFORT.floorFrac;
  C.COMFORT.floorFrac = 0;
  {
    const s = ST.newGame(); s.accommodation.tier = 8; s._comfortCache = 5e5;
    const legacy = 1 + C.COMFORT.MULT * Math.log10(1 + 5e5 / C.COMFORT.C0);
    ok(M.comfortMultiplier(s) === legacy, 'floorFrac=0 ⇒ comfortMultiplier is BIT-IDENTICAL to the pre-refit total-Comfort formula (the regression escape hatch)');
  }
  C.COMFORT.floorFrac = savedFF;

  // (b) ABOVE-FLOOR BEHAVIOR: at floorFrac=1 the accScore backbone cancels, so two states with the
  // SAME above-floor Comfort but different tiers get the SAME income multiplier — a tier-up is
  // neutral (never a penalty), and only player-earned Comfort (amenities/Body) moves income.
  {
    const lo = { _comfortCache: M.accScore(3) + 4000, accommodation: { tier: 3 } };
    const hi = { _comfortCache: M.accScore(9) + 4000, accommodation: { tier: 9 } };
    ok(approx(M.comfortMultiplier(lo), M.comfortMultiplier(hi)),
      'floorFrac=1: equal above-floor Comfort ⇒ equal income multiplier regardless of tier (check-in is neutral, not a drop)');
    const more = { _comfortCache: M.accScore(9) + 40000, accommodation: { tier: 9 } };
    ok(M.comfortMultiplier(more) > M.comfortMultiplier(hi),
      'more above-floor Comfort (amenities/Body) ⇒ strictly higher income multiplier — amenities are the live income lever now');
  }

  // (c) the GATE still reads TOTAL Comfort (unchanged): a state whose Comfort is pure accScore (no
  // amenities) still clears the next tier's unlock gate — accScore self-satisfies (unlockFrac 0.33
  // < 1/growth 0.385), so the above-floor refit CANNOT re-introduce the unbridgeable-wall failure.
  {
    const g = ST.newGame(); g.accommodation.tier = 12;
    g._comfortCache = M.computeComfort(g, DATA);   // accScore(12) + wBody·0, NO amenities
    ok(g._comfortCache >= M.accUnlockComfort(13),
      'the tier GATE reads TOTAL Comfort: owning tier 12 (accScore alone) already clears tier 13 — bridgeability preserved');
    const eff = Math.max(0, g._comfortCache - C.COMFORT.floorFrac * M.comfortFloor(g));
    ok(M.comfortMultiplier(g) === 1 + C.COMFORT.MULT * Math.log10(1 + eff / C.COMFORT.C0),
      'the INCOME multiplier reads ABOVE-FLOOR Comfort while the GATE reads TOTAL — the two roles are cleanly separated');
    ok(M.accUnlockComfort(13) === M.accScore(13) * C.ACC.unlockFrac,
      'accUnlockComfort is floorFrac-free (still accScore·frac) — the gate never sees the income-multiplier floor');
  }
}
...
console.log('\n[56] E11 conciergeTick: budget bound + reserve floor + tip sink');
{
  ...
  const tip = conciergeTestState(170); // budget = 42.5, just enough for bug_spray (cost 20 × costScale 2 = 40)
...
console.log('\n[69] E12 Creator Dashboard reveal gating + Going Viral flourish + cross-path hybrid flavor');
{
  ...
  E.tick(gv, 1); fireStory(gv); E.tick(gv, 0.1);
...
console.log('\n[74] committed-path contract: one road per life, focus/nudges only on the chosen path');
{
  ...
  E.tick(neutral, 1); fireStory(neutral); E.tick(neutral, 0.1);
...
console.log('\n[83] E13 beat 14 crypto variant (Whale Watching) + neutral fallback + Savvy/path bonus');
{
  ...
  E.tick(cr, 1); fireStory(cr); E.tick(cr, 0.1);
...
console.log('\n[85] bank account / wallet cap: ladder data, inflow clamp, offline bound, migration');
{
  ...
  asc.bank.tier = 7;
  asc.stats.lifetimeCashThisTree = 1e12; asc.stats.runSec = 300; asc.accommodation.tier = 12;
...
console.log('\n[86] ascension: hard reset, parabolic gate scaling, gate-invariant Legacy, run-2 pacing');
{
  ...
  // Phase-C refit: the loop's contract changed from "each run longer" (measured as a chore —
  // audit 2.6) to a FLAT PLATEAU: runs 2+ land within [0.85, 1.10]× run 1, so prestige reads
  // as power, not as a longer rerun. Measured arc: 10h56m → 10h13m → 9h34m.
  ok(run2Island >= run1Island * 0.85 && run2Island <= run1Island * 1.10,
    `ascended run holds the flat plateau band 0.85-1.10× run 1 (got ${fmtTime(run2Island)} vs ${fmtTime(run1Island)})`);
  ok(tierT2[5] < run1Island * 0.40, `early tiers hold near run-1 pace under the hardened gate (tier 5 at ${fmtTime(tierT2[5])})`);
...
console.log('\n[87] staged path tracks: thresholds, stage reveals, unique per-path bonuses');
{
  ...
  a.stats.lifetimeCashThisTree = 1e12; a.stats.runSec = 300; a.accommodation.tier = 12;
...
console.log('\n[88] Jack of All Trades: earned path mixing, slot caps, depth, reset behavior');
{
  ...
  m.stats.lifetimeCashThisTree = 1e12; m.stats.runSec = 300; m.accommodation.tier = 12;
...
console.log('\n[89] E14 connoisseur: exclusivity ×, luxury discount, appreciation, gate invariance');
{
  ...
  E.tick(pv, 1); fireStory(pv); E.tick(pv, 0.1);
  ...
  E.tick(nonConn, 1); fireStory(nonConn); E.tick(nonConn, 0.1);
...
console.log('\n[90] E15 logistics: cars/slots/upkeep, logistics × gate invariance, repossession');
{
  ...
  E.tick(b15, 1); fireStory(b15); E.tick(b15, 0.1);
...
console.log('\n[96] E21 Seven Stars: Seven-Star Touches cluster, gated exclusivity spillover, tier 14/15 velvet-rope, patron beats 21/22, invariance');
{
  ...
  ok(SEVEN.some(id => hs.amenities[id].level > 0), 'the harness now BUYS Seven-Star Touches — above-floor Comfort makes them ROI-positive (were dominated cosmetics)');
  // The GATE invariant that E21 actually guards is untouched: tag:luxury exclusivity spillover is
  // connoisseur-gated, so a vlogger harness owning Seven-Star Touches still earns 0 exclusivity.
  ok(M.connoisseurActive(hs) === false && (hs._exclCache ?? 0) === 0, 'the harness stays non-connoisseur ⇒ those Touches add 0 exclusivity (the gate is shut)');
}
...
console.log('\n[100] E25-A The Family Album: lineage retirement, album keep-list, epitaph determinism, no income leak');
{
  ...
  a.stats.lifetimeCashThisTree = 1e12; a.stats.runSec = 300; a.stats.bestComfort = 5e6; a.bank.tier = 3;
  ...
  const b = ST.newGame(); b.stats.lifetimeCashThisTree = 1e12; b.stats.runSec = 300; b.bank.tier = 3;
  P.ascend(b);
  ...
  const k = ST.newGame(); P.setLineageName(k, 'Joost'); k.stats.lifetimeCashThisTree = 1e12; k.stats.runSec = 300; k.bank.tier = 5;
  ...
  cap.stats.lifetimeCashThisTree = 1e12; cap.stats.runSec = 300; cap.bank.tier = 3;
...
console.log('\n[102] E27 The Island Listing: multi-currency purchase, relocation, meta-key persistence, invariance');
{
  ...
  E.buyIsland(asc);
  asc.stats.lifetimeCashThisTree = 1e12; asc.stats.runSec = 300; asc.bank.tier = 3; asc.resources.legacy = 5;
  P.ascend(asc);
...
console.log('\n[104] E29 Empire of Leisure: Legend prestige-2, meta-meta shop, New Game+, invariance');
{
  ...
  const acc = ST.newGame(); acc.stats.lifetimeCashThisTree = 1e12; acc.stats.runSec = 300; acc.bank.tier = 3;
...
console.log('\n[105] E30 Legends of Leisure: achievements/collections, seasonal, the golden file, invariance');
{
  ...
  rec.stats.lifetimeCashThisTree = 1e12; rec.stats.runSec = 300; rec.bank.tier = 3;
  P.ascend(rec);
...
console.log('\n[106] demo playthrough runner (js/dev/demo.mjs): baseline identity, cadence, recorder');
{
  ...
  const crypto = runScenario(getScenario('greedy-crypto'), { dt: 5, maxHours: 3 });
...
console.log('\n[107] demo runner prestige layer: Legacy/Legend spending, ascension loop, stall detection');
{
  ...
  // ---- ascension integration: 12h of the ascension-loop scenario covers island → ascend →
  // spend → generation 2 (the refit greedy core reaches tier 20 at ~10h24m post-Comfort-refit).
  const asc = runScenario(getScenario('ascension-loop'), { dt: 5, maxHours: 14 });
  ok(asc.final.ascensions === 1, `ascension-loop ascends exactly once in 14h (got ${asc.final.ascensions})`);
  ok(asc.islandAt !== null && asc.islandAt > 36000 && asc.islandAt < 39000, `island time recorded despite the same-step reset (${fmtTime(asc.islandAt)})`);
...
console.log('\n[109] Phase B instruments: branch parity, casual band, beat spacing, organic Legend — measured baselines the refit must move (docs: .claude/context/audit-fable-improvements.md Phase C gate)');
{
  ...
  const vg = island('greedy-vlogger', { dt: 5, maxHours: 12 });
  const tr = island('greedy-traveler', { dt: 5, maxHours: 12 });
  const cn = island('greedy-connoisseur', { dt: 5, maxHours: 11 });
  const cr = island('greedy-crypto', { dt: 5, maxHours: 14 });
  ...
  ok(cas.islandAt !== null, `casual-tourist reaches the island (${fmtTime(cas.islandAt)})`);
  console.log(`    → casual-tourist island ${fmtTime(cas.islandAt)} (the ~20h contract, measured)`);
  ok(cas.islandAt >= 18 * 3600 && cas.islandAt <= 22 * 3600, `casual-tourist lands the 18-22h band (got ${fmtTime(cas.islandAt)})`);
  ok(Math.abs(cas.islandAt - 76800) <= 1200, `casual-tourist fitted pin ~21h20m (got ${fmtTime(cas.islandAt)})`);

  // ---- beat spacing: the valve + refit de-clustered the story (audit: 26 beats on 14
  // distinct timestamps, triple-dumps at the emotional peaks). Now every beat has its moment.
  const beatTimes = vg.events.filter(e => e.type === 'beat').map(e => e.t);
  const distinct = new Set(beatTimes).size;
  console.log(`    → ${beatTimes.length} beats fire on ${distinct} distinct timestamps`);
  ok(distinct >= 24, `beat timestamps are distinct (${distinct} ≥ 24 — the valve rations one beat per window)`);

  // ---- organic Legend: first point on the MEASURED post-refit arc (24 + ~9/ascension,
  // see [107]'s loop) crosses LEGEND_SCALE by ascension ≤ 5 — no synthetic state needed.
  const arcTotal = n => 24 + 9 * (n - 1);
  const firstLegendAt = [1, 2, 3, 4, 5, 6].find(n => Math.floor(C.LEGEND_K * Math.pow(arcTotal(n) / C.LEGEND_SCALE, C.LEGEND_EXP)) >= 1);
  console.log(`    → organic Legend: first point at ascension ${firstLegendAt ?? '>6'} on the measured arc (scale ${C.LEGEND_SCALE})`);
  ok(firstLegendAt !== undefined && firstLegendAt <= 6, `Legend is organically reachable by ascension ≤6 (got ${firstLegendAt})`);
  // NG+ nets HARDER per cycle now (gates ×2.2 vs income ×2.0 — the audit measured the old
  // 1.5-vs-3.0 as net ×2 EASIER while the copy claimed hardening).
...
console.log('\n[113] Living-World W3: Souvenir Stand + Ascension Challenges + Legacy Honeymoon — the meta loop, run 1 untouched');
{
  ...
  // reference formula updated for the above-floor refit (docs/05 §9.3): the challenge-free
  // comfortMultiplier now reads effective (above-floor) Comfort. neu is a fresh game (tier 0 ⇒
  // floor accScore(0)·wAcc = 50), so this stays the exact identity when no challenge is active.
  const effNeu = Math.max(0, neu._comfortCache - C.COMFORT.floorFrac * M.comfortFloor(neu));
  const oldComfort = 1 + C.COMFORT.MULT * Math.log10(1 + effNeu / C.COMFORT.C0);
  ok(M.comfortMultiplier(neu) === oldComfort, 'comfortMultiplier is bit-identical to the challenge-free above-floor formula with no active challenge');
  ...
  };
  {
    ...
    s._comfortCache = 5e8;
    // raw uses effective (above-floor) Comfort now — see the above-floor refit note above (fresh
    // game ⇒ tier-0 floor 50); Rainy Season still halves the multiplier's EXCESS over 1.
    const effS = Math.max(0, s._comfortCache - C.COMFORT.floorFrac * M.comfortFloor(s));
    const raw = C.COMFORT.MULT * Math.log10(1 + effS / C.COMFORT.C0);
