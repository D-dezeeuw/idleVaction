...
|---|---|---|
| Holds to 20h for one run? | ⚠️→✅ **Now yes (fitted).** As *originally shipped* it did not — optimal play hit the island in **~4 min** and cash **overflowed `double` at 9m 26s**. After the P0 fix + a degree-suppression fit, the greedy-optimal harness lands the island inside the **6–12h guard band** (see §13 and the selftest golden pin for the current fitted second), peak `~1e11` — the ~18h figure earlier drafts of this row quoted predates the final fit. Casual pacing is measured by the demo runner's cadence policies (selftest [109]). | §2–§6, §10, §13 |
...
| **P1** | Fit `base_k`/`growth_k`/`perUnit_k` to the 20 h schedule via the harness loop | tuning | ✅ **landed** | Greedy-optimal island inside the 6–12h band (`node js/dev/harness.mjs` prints the current fitted value; the golden pin in selftest is canonical — the ~18h quoted here earlier was an intermediate fit), monotone beats; gate spacing + casual-band fit tracked in the Phase-C refit |
